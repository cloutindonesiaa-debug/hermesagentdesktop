...
### Path with spaces in username (the `cd C:\Users\...` auto-prefix bug)

Hermes' shell wrapper auto-prefixes every `terminal` and `write_file` command
with `cd <user.home>` from the system prompt. **When the Windows username
contains a space** (e.g. `C:\Users\Clout Indonesia\`), the prefix arrives
unquoted and bash fails:

```
/bin/bash: line 2: cd: C:\Users\Clout Indonesia: No such file or directory
# or, for write_file:
/bin/bash: -c: line 8: unexpected EOF while looking for matching `"'
```

**Don't conclude the tools are broken** — try these workarounds in order:

1. **Pass `workdir="/tmp"` on every `terminal` call.** The auto-prefix still
   gets injected, but since `cd` to a non-existent Windows path is the first
   command, the **rest of the script still runs** (the prefix line errors
   out with exit 126, but the user's actual command on subsequent lines
   executes fine). This single flag unblocks 90% of the time.
2. **For file writes, switch to the WSL mount path** `/mnt/c/Users/<name with spaces>/...`
   via `cp` from `/tmp/` rather than `write_file` directly. The `/mnt/c/`
   mount handles spaces when paths are double-quoted (`"/mnt/c/Users/Clout Indonesia/..."`).
3. **For multi-line file content, use `cat > <file> << 'DELIM'` heredoc** —
   the *single-quoted* delimiter blocks all bash variable/command
   substitution, so backticks, `$()`, and `${var}` in the content survive
   intact. **Avoid `&` in the content** (the wrapper detects it as
   backgrounding and rejects the whole call with `Foreground command uses '&' backgrounding`).
4. **If `workdir=/tmp` is forgotten and prefix fires anyway**, don't loop
   the same failing command — switch tools (e.g. `terminal` with the flag,
   or `cp` from a previously-written `/tmp/` file).

**When to fix it permanently:** if the user agrees, have them rename the
Windows user folder (e.g. `C:\Users\Clout Indonesia` → `C:\Users\clout-indonesia`)
or create a directory symlink with `mklink /D C:\Users\Clout C:\Users\CloutIndonesia`.
Don't push this — many users have app configs pinned to the spaced path.

Full diagnostic recipe with worked commands: `references/windows-path-spaces.md`.
